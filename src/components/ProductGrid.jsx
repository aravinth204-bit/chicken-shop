...
function ProductGrid({ products, addToCart, searchQuery, setSearchQuery, sortBy, setSortBy, showToast }) {
  ...
  if (products.length === 0) {
    return (
      <section id="products" className="py-20 bg-stone-800">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <div className="w-24 h-24 bg-stone-700 rounded-full flex items-center justify-center mx-auto mb-6">
            <span className="text-5xl">📦</span>
          </div>
          <h3 className="text-2xl font-bold text-white mb-2">No Products Found</h3>
          <p className="text-amber-100/60">Try adjusting your search or filters</p>
        </div>
      </section>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
          <div>
            ...
            <p className="text-amber-100/60 text-sm sm:text-base mt-1">{products.length} items</p>
          </div>

          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full sm:w-auto">
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-amber-100/60" />
              <input
                type="text"
                placeholder="Search..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full sm:w-48 lg:w-64 bg-stone-700/50 border border-stone-600 text-white pl-10 pr-3 py-2.5 sm:py-3 rounded-lg sm:rounded-xl focus:outline-none focus:border-red-500 text-sm"
              />
            </div>
            ...
            <div className="relative">
              <select
                ...
                className="w-full sm:w-40 bg-stone-700/50 border border-stone-600 text-white pl-3 pr-8 py-2.5 sm:py-3 rounded-lg sm:rounded-xl focus:outline-none focus:border-red-500 appearance-none cursor-pointer text-sm"
              >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
          {products.map((product, index) => (
            <div
              key={product.id}
              className="group bg-stone-900/80 backdrop-blur rounded-xl sm:rounded-2xl overflow-hidden border border-stone-700/50 hover:border-red-500/30 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-black/20 animate-fade-in-up"
              style={{ animationDelay: `${index * 75}ms` }}
            >
              <div className="relative h-32 sm:h-40 bg-gradient-to-br from-stone-800 to-stone-900 flex items-center justify-center overflow-hidden">
              ...
              <div className="p-3 sm:p-4">
                ...
                <p className="text-amber-100/60 text-xs sm:text-sm mb-3 sm:mb-4 line-clamp-1">{product.description}</p>

                <div className="mb-3 sm:mb-4">
                  <p className="text-xs text-amber-100/40 mb-2 font-medium uppercase tracking-wide">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {(product.unit === 'piece' ? dozenOptions : weights).map(w => (
                      <button
                        ...
                        className={`px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg text-xs sm:text-sm font-semibold transition-all active:scale-95 ${
